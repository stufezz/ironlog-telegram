...
function initTelegram(){if(!telegram)return;telegram.ready();telegram.expand();if(telegramSupports('7.7'))telegram.disableVerticalSwipes();if(telegramSupports('6.1')){telegram.setHeaderColor('bg_color');telegram.setBackgroundColor('bg_color')}document.addEventListener('click',event=>{if(telegramSupports('6.1')&&event.target.closest('button,label[for]'))telegram.HapticFeedback.selectionChanged()},{passive:true});if(!saved&&telegramSupports('9.0'))try{telegram.DeviceStorage.getItem(STORAGE_KEY,(error,value)=>{if(error||!value)return;try{let data=JSON.parse(value);state.templates=normalizeTemplates(data.templates);state.sessions=normalizeSessions(data.sessions,state.templates);render();toast('Данные Telegram загружены')}catch{}})}catch{}}

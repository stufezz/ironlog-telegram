...
const effortOptions=[
  {value:'easy',title:'Легко',icon:'♧'},
  {value:'normal',title:'Нормально',icon:'◐'},
  {value:'hard',title:'Тяжело',icon:'♨'}
];
...
function normalizeSessions(items,templates=migratedTemplates){
  return (Array.isArray(items)?items:[]).map(session=>{
    ...
    return {
      ...
      effort:effortOptions.some(item=>item.value===session.effort)?session.effort:null,
...
const state={
  ...
  weightProgressExpanded:true,recentHistoryExpanded:true,completedExerciseOrder:[]
};
...
function effortInfo(value){return effortOptions.find(item=>item.value===value)}
function effortBadge(value){let effort=effortInfo(value);return effort?`<span class="effort-badge ${effort.value}">${effort.icon} ${effort.title}</span>`:''}
...
function history(){
  ...
  let rows=done.map(session=>{let index=state.sessions.indexOf(session),date=dateFromISO(session.date);return `<button class="card history-row openHistory" data-session="${index}"><span class="datebox">${date.getDate()}<small>${ruDate(date,{month:'short'}).replace('.','').toUpperCase()}</small></span><span><b>${escapeHTML(session.name)}</b><span class="history-meta"><span class="muted">${countDone(session)} подходов · ${Math.floor(session.durationSeconds/60)} мин</span>${effortBadge(session.effort)}</span></span><span class="open-arrow">›</span></button>`}).join('');
  let recentHeader=done.length?`<button class="section section-toggle" id="toggleRecent"><span>ПОСЛЕДНИЕ ТРЕНИРОВКИ</span><span>${state.recentHistoryExpanded?'Свернуть⌃':'Развернуть⌄'}</span></button>`:'<div class="section">ПОСЛЕДНИЕ ТРЕНИРОВКИ</div>';
  return `<section class="screen">${header('ПРОГРЕСС','История')}<div class="card metric-grid"><div class="metric-big"><b>${done.length}</b><span class="muted">${workoutWord(done.length)}</span></div><div class="metric-big"><b>${totalVolume(done)}</b><span class="muted">общий тоннаж, кг</span></div></div><button class="card file-export" id="openFiles"><span class="file-box">⇩</span><span><b>ФАЙЛЫ И РЕЗЕРВНАЯ КОПИЯ</b><small class="muted">JSON, CSV и восстановление данных</small></span><span class="share-arrow">›</span></button>${programs.length?weightProgress(programs,names,programSessions):''}${recentHeader}${state.recentHistoryExpanded?rows:''}${rows?'':'<div class="card empty">Заверши первую тренировку — здесь появится история и график прогресса.</div>'}</section>${nav('history')}`;
}
function weightProgress(programs,names,sessions){
  let selected=state.chartExercise,program=programs.find(item=>item.key===state.chartProgram),points=sessions.map((session,index)=>{let exercise=session.exercises.find(item=>item.name===selected),sets=exercise?.sets.filter(set=>set.done||Number(set.weight)>0)??[];return sets.length?{weight:Math.max(...sets.map(set=>Number(set.weight)||0)),label:`#${index+1}`}:null}).filter(Boolean),weights=points.map(point=>point.weight),latest=weights.at(-1)??0,delta=weights.length>1?latest-weights.at(-2):0,rawMin=Math.min(...weights),rawMax=Math.max(...weights),min=rawMin===rawMax?Math.max(0,rawMin-5):rawMin,max=rawMin===rawMax?rawMax+5:rawMax,range=Math.max(1,max-min),coords=points.map((point,index)=>({x:points.length===1?160:18+index*(284/(points.length-1)),y:145-((point.weight-min)/range)*112,...point})),polyline=coords.map(point=>`${point.x},${point.y}`).join(' ');
  let content=`<div class="label">ПРОГРАММА</div><div class="chart-tabs">${programs.map((item,index)=>`<button class="chart-tab ${item.key===state.chartProgram?'active':''}" data-chart-program="${index}">${escapeHTML(item.name)}</button>`).join('')}</div>${names.length?`<div class="label">УПРАЖНЕНИЕ</div><div class="chart-tabs">${names.map((name,index)=>`<button class="chart-tab ${name===selected?'active':''}" data-chart-exercise="${index}">${escapeHTML(name)}</button>`).join('')}</div>${points.length?`<svg class="weight-chart" viewBox="0 0 320 175"><line class="grid" x1="18" y1="33" x2="302" y2="33"/><line class="grid" x1="18" y1="89" x2="302" y2="89"/><line class="grid" x1="18" y1="145" x2="302" y2="145"/><polyline class="line" points="${polyline}"/>${coords.map(point=>`<circle class="point" cx="${point.x}" cy="${point.y}" r="6"/><text x="${point.x}" y="${point.y-12}" text-anchor="middle">${point.weight}</text><text x="${point.x}" y="164" text-anchor="middle">${point.label}</text>`).join('')}</svg><span class="muted">${points.length===1?'Добавь ещё одну тренировку, чтобы увидеть динамику.':'Максимальный записанный вес по каждой тренировке.'}</span>`:'<div class="empty">Для этого упражнения пока нет сохранённых весов.</div>'}`:'<div class="empty">В этой программе пока нет сохранённых упражнений.</div>'}`;
  return `<div class="card"><button class="chart-heading" id="toggleWeight"><span><span class="eyebrow">ПРОГРЕСС ВЕСА</span><b>${escapeHTML(program?.name||'Программа')}</b><small class="muted">${escapeHTML(selected||'')}</small></span><span class="chart-summary"><b>${latest} кг</b><small class="muted">${weights.length>1?`${delta>=0?'+':''}${delta} кг · `:''}${state.weightProgressExpanded?'Свернуть⌃':'Развернуть⌄'}</small></span></button>${state.weightProgressExpanded?content:''}</div>`;
}
...
function bind(){
  ...
  $('#toggleWeight')?.addEventListener('click',()=>{state.weightProgressExpanded=!state.weightProgressExpanded;render()});
  $('#toggleRecent')?.addEventListener('click',()=>{state.recentHistoryExpanded=!state.recentHistoryExpanded;render()});
...
function bindSession(){
  ...
  document.querySelectorAll('[data-rest-seconds]').forEach(button=>button.onclick=()=>startRestTimer(Number(button.dataset.restSeconds)));
  $('#stopSessionRest')?.addEventListener('click',()=>stopRestTimer());
  ...
  document.querySelectorAll('[data-check]').forEach(button=>button.onclick=()=>{let [exerciseIndex,setIndex]=button.dataset.check.split(',').map(Number),exercise=state.active.exercises[exerciseIndex],set=exercise.sets[setIndex];set.done=!set.done;state.completedExerciseOrder=state.completedExerciseOrder.filter(id=>id!==exercise.id);if(exercise.sets.length&&exercise.sets.every(item=>item.done))state.completedExerciseOrder.push(exercise.id);persist();render()});
...
function startTemplate(index,date){
  ...
  let exercises=template.exercises.map(item=>({id:uid(),name:item.name,notes:'',sets:Array.from({length:item.sets},(_,setIndex)=>({id:uid(),weight:0,reps:item.reps,rir:item.rir,done:false,number:setIndex+1}))}));
  let session={id:uid(),templateId:template.id,name:template.name,date,done:false,completedAt:null,durationSeconds:0,notes:'',effort:null,exercises};state.sessions.push(session);state.active=session;state.sessionOpenedAt=Date.now();state.editingCompleted=false;state.completedExerciseOrder=[];state.screen='session';persist();if(telegramSupports('6.1'))telegram.HapticFeedback.impactOccurred('medium');render();
}
function openSession(index){let session=state.sessions[index];if(!session)return;state.active=session;state.sessionOpenedAt=Date.now();state.editingCompleted=false;state.completedExerciseOrder=[];state.screen='session';render()}
...
function confirmFinish(){app.insertAdjacentHTML('beforeend',`<div class="modal" style="display:grid;align-content:center"><div class="card confirm-card"><div class="eyebrow">КАК ПРОШЛА ТРЕНИРОВКА?</div><h2>Выбери общую сложность</h2><p class="muted" style="font-size:13px;margin-bottom:20px">Отмечено подходов: ${countDone(state.active)}.</p><div class="effort-options">${effortOptions.map(effort=>`<button data-finish-effort="${effort.value}" class="${effort.value}"><span>${effort.icon}</span><b>${effort.title}</b></button>`).join('')}</div><button class="iconbtn" id="cancelConfirm" style="width:100%;margin-top:12px">Отмена</button></div></div>`);$('#cancelConfirm').onclick=()=>$('.modal').remove();document.querySelectorAll('[data-finish-effort]').forEach(button=>button.onclick=()=>finishWorkout(button.dataset.finishEffort))}
function finishWorkout(effort){let session=state.active;session.durationSeconds=sessionDuration(session);session.done=true;session.completedAt=new Date().toISOString();session.effort=effort;state.sessionOpenedAt=Date.now();stopRestTimer(false);state.editingCompleted=false;state.screen='history';persist();$('.modal')?.remove();render();toast('Тренировка сохранена')}
...
function startRestTimer(seconds=90){state.restEnd=Date.now()+Math.max(1,Number(seconds)||90)*1000;state.restSessionId=state.active?.id||null;persist();render()}
